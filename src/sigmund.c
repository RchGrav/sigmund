...
static int write_record_atomic(const char *dir, const struct record *r,
                               int argc, char **argv, char *out_json_path,
                               size_t out_n) {
  char tmp[1200], fin[1200];
  if (checked_snprintf(fin, sizeof(fin), "%s/%s.json", dir, r->id) != 0) {
    return -1;
  }
  if (checked_snprintf(tmp, sizeof(tmp), "%s/.%s.tmp.%ld", dir, r->id,
                       (long)getpid()) != 0) {
    return -1;
  }
...
static int append_cmd_escaped(char *dst, size_t n, size_t *off,
                              const char *arg) {
  const char *sq = "'\\''";
  if (*off + 1 >= n) {
    return -1;
...
static int count_session_escapees(pid_t sid, pid_t expected_pgid) {
  ...
  while ((e = readdir(d))) {
    ...
    if (checked_snprintf(path, sizeof(path), "/proc/%ld/stat", (long)pid) !=
        0) {
      continue;
    }
    int fd = open(path, O_RDONLY);
...
static void report_session_escapees(const struct record *r) {
  ...
  if (escaped > 0) {
    fprintf(stderr,
            "sigmund: warning: %d process(es) escaped process-group %ld but "
            "remain in session %ld\n",
            escaped, (long)r->pgid, (long)r->sid);
  }
}

static int read_proc_stat_tokens(pid_t pid, char *state_out,
                                 uint64_t *starttime_out) {
  char path[128], buf[4096];
  ...
  for (char *tok = strtok_r(p, " ", &save); tok;
    ...
    if (idx == 19 && starttime_out) {
      char *end = NULL;
      errno = 0;
      unsigned long long starttime = strtoull(tok, &end, 10);
      if (end == tok || errno != 0) {
        return -1;
      }
...
static enum run_state eval_state(const struct record *r,
  ...
  if (present) {
    ...
    if (r->exe_dev && r->exe_ino) {
      uint64_t d, i;
      if (read_proc_exe(r->pid, &d, &i) == 0 &&
          (d != r->exe_dev || i != r->exe_ino)) {
        return STATE_STALE;
      }
    }
...
static int perform_start(const char *dir, bool persistent, int argc,
                         char **argv) {
  ...
  if (pid == 0) {
    ...
    if (!interactive) {
      ...
      if (lfd < 0 || dup2(lfd, STDOUT_FILENO) < 0 ||
...
static int load_record_by_id(const char *dir, const char *id, struct record *r,
                             char *path, size_t n) {
  if (!valid_id(id)) {
    return -1;
  }
...
static int do_signal_action(const char *dir, const char *id, int sig,
                            bool graceful) {
  struct record r;
  ...
  if (r.has_boot && get_boot_id(boot, sizeof(boot)) == 0 &&
      strcmp(r.boot_id, boot) != 0) {
    return 2;
  }
  ...
  if (graceful) {
    ...
    while (waited < STOP_TIMEOUT_MS) {
      ...
      char state = 0;
      if (g == 0 ||
          (read_proc_stat_tokens(r.pgid, &state, NULL) == 0 && state == 'Z')) {
...
static int cmd_list(const char *dir) {
  ...
  while ((e = readdir(d))) {
    ...
    printf("%-7s %-8ld %-8ld %-6s %-8s %s\n", r.id, (long)r.pid, (long)r.pgid,
           age, state_str(st), cmd);
  }
  closedir(d);
  return 0;
}
...
static int cmd_prune(const char *dir) {
  ...
  while ((e = readdir(d))) {
    ...
    if (checked_snprintf(json_path, sizeof(json_path), "%s/%s.json", dir, id) !=
        0) {
      continue;
    }
    if (checked_snprintf(log_path, sizeof(log_path), "%s/%s", dir, e->d_name) !=
        0) {
      continue;
    }
